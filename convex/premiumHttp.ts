...
// hi-IN is Hindi practice: Azure assesses it at word level, and the TTS voice
// reads the Devanagari word it is given.
const wordSchema = z.object({ word: z.string().trim().min(1).max(40), locale: z.enum(['en-US', 'en-GB', 'en-AU', 'en-CA', 'en-IN', 'hi-IN']).default('en-US') }).strict();
const locales = new Set(['en-US', 'en-GB', 'en-AU', 'en-CA', 'en-IN', 'hi-IN']);
const errors: Record<string, [number, string]> = {
  account_deleting: [403, 'Account deletion has started. Finish deleting your account in Settings.'],
  authentication_required: [401, 'Sign in to use personal feedback.'], upgrade_required: [403, 'SpeakWell Pro unlocks this feature.'],
...
export async function processPremium(ctx: ActionCtx, request: Request, feature: Exclude<PremiumFeature, 'analytics'>): Promise<Response> {
  ...
  try {
    ...
    if (feature === 'exercise') {
      ...
      const generated = await generateObject({ model: process.env.AI_PASSAGE_MODEL || 'google/gemini-3.5-flash-lite', system: passage.systemPrompt(data.language),
