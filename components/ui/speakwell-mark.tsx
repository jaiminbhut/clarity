...
export function SpeakWellMark({ size, tone = 'primary' }: { size: number; tone?: 'primary' | 'marketing' }) {
  const { colors } = useTheme();
  const ink = tone === 'marketing' ? colors.marketingInk : colors.foreground;

  return (
    <Svg width={size} height={size} viewBox="0 0 873 812" accessibilityRole="image" accessibilityLabel="SpeakWell">
